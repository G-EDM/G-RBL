    ...
    if( !line.skip_floating ){
        ...
        if( direction ){
            if(
                ...
                && ( microseconds - last_step_micros[Z_AXIS] > process_speeds.RAPID )
...
void G_EDM_PLANNER::pause( bool redraw ){
    ...
    //_state.wire_has_first_contact = false;
    ui_controller.reset_flush_retract_timer();
    if( ! ui_controller.start_edm_process() || simulate_gcode ){ 
        is_paused = false;
        return; 
    }
    was_paused          = true;
...
bool IRAM_ATTR G_EDM_PLANNER::short_circuit_estop(){
    ...
    //########################################################
    if( short_circuit_start_time != 0 ){
        if ( esp_timer_get_time() - short_circuit_start_time > adc_critical_conf.short_circuit_max_duration_us ){ 
            gconf.edm_pause_motion = true; 
...
void G_EDM_PLANNER::configure(){
    ...
    no_workload_steps_to_finish = round( finish_after_mm * g_axis[sinker_axis]->steps_per_mm.get() );
...
//###################################################################
// sets the cutting depth relative to the probe position
// 0.0 = no limit 
// this function doesn't care if the limit set is out of range! #todo
// only needed for floating z which is currently disabled 
//###################################################################
void G_EDM_PLANNER::set_cutting_depth_limit( float _mm ){
    ...
    int cutting_depth_steps  = round( _mm * g_axis[sinker_axis]->steps_per_mm.get() ); 
...
bool G_EDM_PLANNER::position_history_move_forward( bool no_motion, Line_Config &line ){
    ...
    //Serial.print( "position_history_move_forward " );
    //Serial.print( position_history[position_history_index_current][X_AXIS] );
    //Serial.println("");
...
bool IRAM_ATTR G_EDM_PLANNER::process_stage( int32_t* target, Line_Config &line ){
    ...
    //Serial.print( "after push_to_position_history " );
    //Serial.print( target[X_AXIS ] );
    //Serial.println("");

    bool _success  = position_history_sync( line );
    //if( !_success ){ gconf.edm_pause_motion = true; }
...
void IRAM_ATTR G_EDM_PLANNER::convert_target_to_steps( float* target, int32_t* __target ){
    for( int axis=0; axis<N_AXIS; ++axis ) {
        ...
        //__target[axis] = lround( target[axis] * g_axis[axis]->steps_per_mm.get() );
        /*Serial.println( "X pos target mm: " );
        Serial.print( target[X_AXIS] );
        Serial.print( "    X pos target step: " );
        Serial.print( __target[X_AXIS] );
        Serial.println("--------------------");
        Serial.println("");*/
    }
}
...
//X: -117.06 / -449496    Y: -140.08 / -537925    
//X current: -449647    
// X target: -2146507464 --- steps per mm: 3840---
// -449510.4
...
void G_EDM_PLANNER::z_axis_up(){
    ...
    int max_pos       = round( DEFAULT_HOMING_PULLOFF * g_axis[sinker_axis]->steps_per_mm.get() ) * -1;
    int offset        = round( DEFAULT_HOMING_PULLOFF * g_axis[sinker_axis]->steps_per_mm.get() );
...
bool G_EDM_PLANNER::z_axis_move_mm( float mm ){
    ...
    int steps         = round( mm * g_axis[Z_AXIS]->steps_per_mm.get() );
    //int steps         = round( mm * axis_settings[Z_AXIS]->steps_per_mm->get() );
    int32_t __target[MAX_N_AXIS]; // work target
    memcpy(__target, sys_position, sizeof(sys_position));
    __target[Z_AXIS] += steps;
    int max_pos = round( DEFAULT_HOMING_PULLOFF * g_axis[Z_AXIS]->steps_per_mm.get() ) * -1;
...
//#############################################################
// Set the direction for a sinker job 1= positive 0= negative
// Does not set the axis itself. Just the direction.
//#############################################################
...
bool IRAM_ATTR G_EDM_PLANNER::do_flush_if_needed(){
    if( ui_controller.check_if_time_to_flush() ){  // time for flush. If flushing intervall time is set to 0 it will disable flushing
        gconf.gedm_flushing_motion = true;         // flag to inform that a flushing motion is performed
        float flush_offset_mm      = 0.0;//1.0 / axis_settings[sinker_axis]->steps_per_mm->get() * float( flush_offset_steps ); // after the retraction is finished the machine will move back to the initial position minus the offset steps to prevent contact
        retraction( sinker_axis, flush_retract_mm, flush_offset_mm, disable_spark_for_flushing ); // do the retraction
        ui_controller.reset_flush_retract_timer(); // reset the timer
...
//##################################################################
// Perform a flushing retraction for sinker jobs
// Note: When a sinker job is started it will set the max travel
// as previous line in the history so that the planner has somewhere
// to retract to. We retract within those boundaries but after a 
// retraction is done the planner will enter a hository retraction
// to prevent messing with anything the machine will think it just 
// did a normal history retraction and and move on
// even if there is no offset used the retraction may not return 
// to the exact same step and this would totally ruin the step loop
// as this function is called while actually running a line
//##################################################################
bool G_EDM_PLANNER::retraction( int stepper, float travel_mm, float offset_mm, bool disable_pwm ){

    if( disable_pwm ){
        toggle_pwm( true, 0 ); // turn spark off while retracting
    }

    // get the current position and save the position for backup usage
    int32_t __backup_position[MAX_N_AXIS]; // stores the initial position to move back to after the retraction
    int32_t __target[MAX_N_AXIS];          // stores the retraction target position

    memcpy(__backup_position, sys_position, sizeof(sys_position)); // sys_position is step pos current into negative space
    memcpy(__target,          sys_position, sizeof(sys_position));

    // convert the travel and offset into steps and store the current step position for the axis
    int __offset_steps      = flush_offset_steps;//round( offset_mm * axis_settings[sinker_axis]->steps_per_mm->get() ); // number of offset steps for return
    int start_step_position = __backup_position[sinker_axis];                                       // log start position for this axis
    int axis_target_step;


    if( _state.sinker_direction == 0 ){

        //##################################################
        // Sinker Job in negative direction
        // Retraction towards home into positive direction
        //##################################################

        // get the max possible travel in positive direction towards home
        float travel_left_positive = api::get_max_travel_possible( sinker_axis, 0 ); // example: 30mm left for positive travel; positive return value or 0;

        // check if wanted travel is within the possible range
        // and fallback to the max possible if it is out of range
        if( travel_mm > travel_left_positive ){
            travel_mm = travel_left_positive;
        }

        // convert travel_mm into steps
        int travel_steps = round( travel_mm * g_axis[sinker_axis]->steps_per_mm.get() );
        //int travel_steps = round( travel_mm * axis_settings[sinker_axis]->steps_per_mm->get() );

        // set the target step by adding a positive number of steps to the negative space current step pos
        axis_target_step = __target[sinker_axis] + travel_steps; // retraction target step position, more positive


    } else {

        //##################################################
        // Sinker Job in positive direction
        // Retraction away from home into negative direction
        //##################################################

        // get the max possible travel in negative direction away from home
        float travel_left_negative          = api::get_max_travel_possible( sinker_axis, 1 ); // example: -30mm left for negative travel; negative return value or 0;
        float travel_left_negative_unsigned = travel_left_negative * -1;                      // remove negativity and use the zen operator *-1

        // check if wanted travel is within the possible range
        // and fallback to the max possible if it is out of range
        if( travel_mm > travel_left_negative_unsigned ){
            travel_mm = travel_left_negative_unsigned;
        }
        ...
        // set the target step by adding a positive number of steps to the negative space current step pos
        axis_target_step = __target[sinker_axis] - travel_steps; // retraction target step position, more negative

    }

    //############################################################
    // Target step is set now
    // Time to create the line package and perform the retraction
    //############################################################

    Line_Config line;                              // basic line with default config
    line.step_delay        = process_speeds.RAPID; // set rapid speed
    line.ignore_feed_limit = true;                 // not needed but set

    __target[sinker_axis]  = axis_target_step;     // overwrite the target position

    //############################################################
    // Rund the line directly. Line is not added to the history!
    //############################################################
    move_line( __target, line );

    //##############################################################
    // Machine is now at the retraction position
    // and needs to drive back to the initial position now
    // This time we work with the backup position created at the
    // beginning
    //##############################################################
    ...
    if( _state.sinker_direction == 0 ){
        ...
        //###########################################################
        // Retraction was done towards home into positive direction
        // Move back into negative
        //###########################################################
        ...
        //###########################################################
        // Retraction was done away from home into negative direction
    ...
    //###########################################################
    // Adjust the line config package
    //###########################################################
    line.exit_on_contact = true;                 // deprecated?
    line.step_delay      = process_speeds.RAPID; // inc ase the speed was overwritten reset it to rapid speed

    //###########################################################
    // Run the return line to the offset position
    //###########################################################

    if( disable_pwm ){ 
        toggle_pwm( false, 0 ); // re-enable PWM if it was disabled
    }
            ...
            if( !gconf.gedm_retraction_motion ){
                    ...
                    if( line.enable_flushing ){
                        if( do_flush_if_needed() ){
                            ...
                            // the machine may not land back at the exact position and therefore all the math done till now
                            // would crippled and hard to correct
                            // this is only done in sinker mode and the previous line to the sinker line is always the
                            // max or min position and a retraction will operate within this space
                            // entering a retraction after a flush may not actually retract further if no short is ongoing
                            // but it will redo the math to move to the final target again
                            _success = false;
                            break;
                        }
                    }
...
uint8_t IRAM_ATTR G_EDM_PLANNER::plan_history_line( float* target, plan_line_data_t* pl_data ) {
    ...
    if( pl_data->is_arc ){
        ...
        line.is_arc = false;
        //if( _state.arc_counter > 0 ){arc_end = true;}
        _state.arc_counter = 0;
    }
    ...
    /*if( _state.arc_counter == 1 ){
        Serial.println("Arc begin");
    } 
    if( arc_end ){
...
X: -117.10 / -449647    Y: -140.40 / -539138    

X current: -449735    X target: 3 --- steps per mm: 3840---

X: -117.06 / -449496    Y: -140.08 / -537925    

X current: -449647    X target: -2146507464 --- steps per mm: 3840---

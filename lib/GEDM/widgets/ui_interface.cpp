...
//############################################################################################################
const char *suffix_table[]                          = {"", " kHz", " %", " mm", " mm/min", " RPM", " kSps", " Lines", " ADC", " us", " V", " A", " s", "ms", 0};
...
void PhantomUI::loader_screen(){
    ...
    canvas.deleteSprite();
    String release = String( RELEASE_NAME );
    canvas.setTextSize(2);
    canvas.createSprite( 320, canvas.fontHeight() );
    canvas.fillScreen(TFT_BLACK);
    canvas.setTextColor(COLORORANGERED);
    canvas.setTextSize(2);
    canvas.drawString(release, 20, 0, 2);vTaskDelay(1);
...
void PhantomUI::touch_calibrate( bool force ){
    ...
    emit(LOG_LINE,"Loading touch data");
    size_t   array_size = 5;
    uint8_t  success    = 0;
    ...
    if( p.begin( RELEASE_NAME ) ){
        ...
        if (REPEAT_DISPLAY_CALIBRATION || force){
            emit(LOG_LINE,"  repeat calibration");
            ...
            if( bytesRead == array_size * sizeof(uint16_t) ) {
                // found the data
                success = 1;
                emit(LOG_LINE,"  touch data loaded");
            } else {
                // no data found
            }
        }
    ...
    if (success && !REPEAT_DISPLAY_CALIBRATION){
        emit(LOG_LINE,"  set calibration data");
        tft.setTouch(calibration_data);
    } else {
        emit(LOG_LINE,"  redo calibration");vTaskDelay(1000);
        fill_screen(TFT_BLACK);
        ...
        if (bytes_write == array_size * sizeof(uint16_t)) {
            emit(LOG_LINE,"  cal file created");
        } else {
            emit(LOG_LINE,"  cal file not created");
        }
        fill_screen(TFT_BLACK);
    }
...
void create_settings_page(){
    ...
    ++current_tab;
    page->select_tab( current_tab ); 
    std::shared_ptr<PhantomWidget> settings_4_ptr = factory_split_list( 320, 205, 0, 0, 2, false );
    ...
    settings_4_ptr->addItem(OUTPUT_TYPE_BOOL,DICT_SPINDLE_ENABLE,PARAM_TYPE_BOOL,PARAM_ID_SPINDLE_ENABLE,0,0,0,0,true,DEFAULT_FONT)
           ->set_colors(1,BUTTON_LIST_BG,TFT_ORANGE)->on(LOAD,load).on(TOUCHED,change_setting);
    settings_4_ptr->addItem(OUTPUT_TYPE_STRING,DICT_SPINDLE_SPEED,PARAM_TYPE_FLOAT,PARAM_ID_SPINDLE_SPEED,0,0,0,0,true,DEFAULT_FONT)
           ->set_colors(1,BUTTON_LIST_BG,TFT_ORANGE)->set_suffix(5) //5=RPM
           ->on(LOAD,load).on(TOUCHED,change_setting);
...
void PhantomUI::build_phantom(){
    ...
    SD_CARD.init( tft.getSPIinstance() ); // it will check for new firmware on init

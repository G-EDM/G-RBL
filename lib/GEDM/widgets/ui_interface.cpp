...
const std::function<void(int *data)> firmware_update_done = [](int *data) {
    ui.emit(LOG_LINE,"Firmware update done!");
    ui.emit(LOG_LINE,"Flagging NVS storage");
    Preferences p;
...
void PhantomUI::build_phantom(){
    ...
    ui.on(FIRMWARE_UPDATE_DONE,firmware_update_done);

...
// 
//##################################################################
void PhantomSD::init( SPIClass &_spi ){
    ...
    __spi     = _spi;
    int error = SD_OK;
    if( !enable_sd_support ){
        state = SD_NOT_ENABLED;
        emit(LOG_LINE,sd_messages[SD_NOT_ENA]);
    } else {
        error = connect();
        if(error){
            emit(LOG_LINE,sd_messages[SD_NOT_FND]);
        } else {
            error = build_directory_tree();
        }
    }
    ready = true;
}
...
int PhantomSD::create_folder( const char* folder ){
    ...
    if( rounds == 0 ){
        emit(LOG_LINE,sd_messages[SD_DIR_EXIST]);
        emit(LOG_LINE,folder);
    }
    return error;
}
...
bool PhantomSD::firmware_update(){
    ...
    if( open_file( firmware_bin, FILE_READ ) == SD_OK ){
	    ...
	    if( Update.end() ){
            emit(LOG_LINE,sd_messages[SD_UPDATE_OK]);
	    } else { //error
            emit(LOG_LINE,sd_messages[SD_UPDATE_FAIL]);
	    }
        close_active();
        if( exists(firmware_bak) ){ // remove existing .bak file
            SD.remove(firmware_bak);
        }
	    if( SD.rename( firmware_bin, firmware_bak ) ){
            emit(LOG_LINE,sd_messages[SD_RENAME_FW]);
        } else {
            emit(LOG_LINE,sd_messages[SD_RENAME_OK]);
        }
        int data = 1;
        ui.emit(FIRMWARE_UPDATE_DONE, &data);
        emit(LOG_LINE,sd_messages[SD_REBOOT]);
        delay(2000); 

...
void IRAM_ATTR ui_task(void *parameter){
    ...
    ui_controller->init(); // this inits the phantom ui etc.
    ...
    ui_controller->set_wire_steps_per_mm( WIRE_SPINDLE_STEPS_PER_MM ); // will be overwritten if there is a value in the NVS
    ...
    ui.start(); // this function will also restore the last session and stored params!
...
void G_EDM_UI_CONTROLLER::pre_process_defaults(){
    ...
    int vfd_drop_adc = adc_critical_conf.voltage_feedback_treshhold;
    int vfd_measured = collect_vsense();
    if( vfd_measured <= vfd_drop_adc ){
        ui.emit( LOG_LINE, "* Adjust vFd *" );
        ui.emit( LOG_LINE, "* vDrop threshold (ADC) *" );
        ui.emit( LOG_LINE, "* needs to be smaller then *" );
        ui.emit( LOG_LINE, "* the measured vfd reading *" );
        vTaskDelay(3000);
    }
...
void set_jog_axis_wire_negative( setter_param * item ){
    ui_controller.manual_motion( char_to_float( item->second ), true );
}
void set_jog_axis_wire_positive( setter_param * item ){
    ui_controller.manual_motion( char_to_float( item->second ) );
}
...
void get_spindle_steps_per_mm( getter_param * item ){
    item->intValue = ui_controller.get_wire_steps_per_mm();
}
void set_spindle_steps_per_mm( setter_param * item ){
    ui_controller.set_wire_steps_per_mm( char_to_int( item->second ) );
}
void get_spindle_speed( getter_param * item ){
    item->floatValue = ui_controller.convert_frequency_to_mm_min( float( wire_spindle_speed ) );
}
// set spindle speed; float value mm/min
void set_spindle_speed( setter_param * item ){
    wire_spindle_speed = ui_controller.convert_mm_min_to_frequency( char_to_float( item->second ) );
    ui_controller.set_speed( wire_spindle_speed );
}
void get_spindle_speed_probing(  getter_param * item  ){
    item->floatValue = ui_controller.convert_frequency_to_mm_min( float( wire_spindle_speed_probing ) );
}
void set_spindle_speed_probing(  setter_param * item  ){
    wire_spindle_speed_probing = ui_controller.convert_mm_min_to_frequency( char_to_float( item->second ) );
}
...
1. convert spindle speed from rpm to mm/min
2. set spindle stepper steps per wire mm

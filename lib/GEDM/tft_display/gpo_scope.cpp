...
void IRAM_ATTR scope_task( void * parameter ){
    ...
    for(;;){
      ...
      //Serial.println("gscope");Serial.println( uxTaskGetStackHighWaterMark(NULL) );vTaskDelay(10);
...
IRAM_ATTR void GPO_SCOPE::setup(){
    if( ! gpo_data.has_task_queue ){
        int max_rounds = 10;
        while( scope_queue == NULL && --max_rounds > 0 ){
            scope_queue = xQueueCreate( 1, sizeof(int16_t)*queue_item_size );vTaskDelay(5);
        }
        xTaskCreatePinnedToCore( scope_task, "scope_task", STACK_SIZE_SCOPE, this, TASK_SCOPE_PRIORITY, &scope_task_handle, SCOPE_TASK_CORE );
    }
...
IRAM_ATTR void GPO_SCOPE::draw_scope(){
    //if( gconf.edm_pause_motion && gconf.process_error != 0 ){ draw_error( gconf.process_error ); }

...
void G_EDM_SENSORS::sensor_end(){

    stop_i2s();

    vTaskDelete( main_sensor_task );
    vTaskDelete( vsense_tasks[0] );

    vQueueDelete( vsense_queue );
    vQueueDelete( motion_plan_queue );
    vQueueDelete( sensor_queue_main );
...
void G_EDM_SENSORS::init_adc(){
    ...
    //###################################################
    int max_rounds = 10;
    while( vsense_queue == NULL && --max_rounds > 0 ){ vsense_queue = xQueueCreate( 1,  sizeof(int) ); vTaskDelay(5); }
    max_rounds = 10;
    while( motion_plan_queue == NULL && --max_rounds > 0 ){ motion_plan_queue  = xQueueCreate( 1,  sizeof(int) ); vTaskDelay(5); }
    max_rounds = 10;
    while( sensor_queue_main == NULL && --max_rounds > 0 ){ sensor_queue_main  = xQueueCreate( 20,  sizeof(int) ); vTaskDelay(5); }
    //###################################################
    // Start tasks
    //###################################################
    xTaskCreatePinnedToCore( default_sensor_task, "sensor_task", STACK_SIZE_A, this, 
        1,//TASK_SENSORS_DEFAULT_PRIORITY, 
        &main_sensor_task, I2S_TASK_A_CORE);

    xTaskCreatePinnedToCore( vsense_task_listener, "vsense_task_listener", STACK_SIZE_B, this, 
        TASK_VSENSE_RECEIVER_PRIORITY, 
        &vsense_tasks[0],  I2S_TASK_B_CORE); 
}
...
bool IRAM_ATTR I2S_is_ready( bool include_all ){
    ...
    } else if( !i2sconf.I2S_READY || i2sconf.I2S_is_restarting ){ 
        return false; 
    }
    return true;
}
void IRAM_ATTR stop_i2s( void ){
    if( !i2sconf.I2S_READY ){ return; }
    i2sconf.I2S_READY = false;
    while( i2sconf.I2S_busy ){
        vTaskDelay(1);
    }
    i2sconf.I2S_READY = false;
    if( scope_queue != NULL ){ xQueueReset( scope_queue ); }
    i2s_stop(I2S_NUM_0);
    i2s_adc_disable(I2S_NUM_0);
    i2s_driver_uninstall( I2S_NUM_0 );
    //Serial.println("I2S not running");
}
void IRAM_ATTR restart_i2s() {
    //i2sconf.I2S_FORCE_RESTART = false;
    if( i2sconf.I2S_is_restarting ){ 
        i2sconf.I2S_FORCE_RESTART = false;
        return;
    }
...
void IRAM_ATTR G_EDM_SENSORS::generate_setpoint_min_max_adc( int adc_reading_reference ){
    ...
    // check if setpoint min is within range
    if( vsense_drop_range_min <= 0.0 ){ vsense_drop_range_min = 1.0; }
    if (vsense_drop_range_min >= vsense_drop_range_max){
        vsense_drop_range_min = vsense_drop_range_max - 0.1;
    }
    // check if setpoint max is within range
    if( vsense_drop_range_max <= vsense_drop_range_min ){ vsense_drop_range_max = vsense_drop_range_min + 0.1; }
    if( vsense_drop_range_max > 100.0 ){
        vsense_drop_range_max = 100.0;
    }
...
// checks if alarm state, stop_process flagged, motioncancel set
...
bool IRAM_ATTR G_EDM_SENSORS::free_edm_start_stop(){
...
IRAM_ATTR bool G_EDM_SENSORS::edm_start_stop(){
    ...
    if (digitalRead(ON_OFF_SWITCH_PIN)){
        ...
        if (edm_process_is_running){
            stop_edm_task = 1;
            sensors.block_on_off_switch = true;
        }
        if (!sys_rt_exec_state.bit.motionCancel){
            force_redraw = true;
        }
...
void G_EDM_SENSORS::enable_hw_timer( int _val ){
    if( !adc_hw_timer.use_hw_timer ){
        if( adc_hw_timer.timer_is_running ){ disable_hw_timer(); }
        return;
    }
...
void IRAM_ATTR default_sensor_task(void *parameter){
    ...
    for (;;){
        ...
        //Serial.println("sensormain");Serial.println( uxTaskGetStackHighWaterMark(NULL) );vTaskDelay(1);
        switch ( data )
        {
            case 1:
            ...
            case 15:
                vTaskDelay(1000); 
                ui_controller.emit(PARAM_ID_ESP_RESTART,&data);
                break;
            ...
            /*case 17:
                G_EDM_SENSORS::free_edm_start_stop();
                G_EDM_SENSORS::edm_start_stop();
                break;*/
...
int IRAM_ATTR G_EDM_SENSORS::collect_vsense( int counts ){
    uint32_t voltage_adc = 0;
    uint32_t adc_counts  = 0;
    int i = 0;
    while( i <= counts ){
        if( sys.abort || sys_rt_exec_state.bit.motionCancel ){ break; }
        if( i2sconf.I2S_busy ){ continue; }
        ++i;
        if( i2s_shift_out() ){
            if( adc_data.voltage_channel_adc_recent > 0 ){
                voltage_adc += adc_data.voltage_channel_adc_recent;
                ++adc_counts;
            }
            vTaskDelay(1);
        }
...
bool IRAM_ATTR G_EDM_SENSORS::adc_to_scope( bool recent ){
    ...
    if( !gscope.is_blocked() && scope_queue != NULL ){
        //Serial.println(1);
...
bool IRAM_ATTR i2s_shift_out(){
    ...
    if( i2sconf.I2S_busy || !I2S_is_ready() ){ 
        return true; 
    }
    ...
    if( ESP_OK == i2s_read(I2S_NUM_0, &adc_batch, ( int ) i2sconf.num_bytes, &bytes_read, I2S_TIMEOUT_TICKS) ){
        i2s_data.samples_read = bytes_read >> 1;
    ...
    if( i2s_data.success && !i2sconf.block_write_out ){
        ...
        //###########################################################################
        i2s_data.sample           = 0;
        i2s_data.selected_channel = 0;

        for (int i = 0; i < i2s_data.samples_read; i++){

                i2s_data.selected_channel = (adc_batch[i] >> 12) & 0x07;
                i2s_data.sample           = adc_batch[i] & 0xfff;
                ...
                if( i2s_data.selected_channel == 6 ){ // current sense
                    ...
                    ++i2s_data.count_b;
                    if( i2s_data.sample < adc_critical_conf.voltage_feedback_treshhold ){
...
void IRAM_ATTR vsense_task_listener(void *parameter){ 
    ...
    for(;;){
        ...
        //################################################################
        pool_size = gconf.gedm_retraction_motion ? i2sconf.I2S_pool_size+1 : i2sconf.I2S_pool_size;
        if( 
            ++i2s_data.num_samples_in_pool <= pool_size 
        ){
            i2s_data.highest_adc = MAX( adc_data.recent, i2s_data.highest_adc );
            if( 
                //!sfeedback.pin_is_low && // vFd indicates a short. No need to collect more samples here
                i2s_data.highest_adc <= setpoints.lower 
...
/*bool     ref_vfd_create    = false;
int      ref_vfd_index     = 0;
int      ref_vfd_buff_size = 20;
uint32_t ref_vfd_tresh     = 0;
uint64_t ref_vfd_buffer;
void generate_ref_vfd( uint32_t sample ){
    ref_vfd_buffer += sample;
    if( ++ref_vfd_index >= ref_vfd_buff_size ){
        ref_vfd_create = false;
        ref_vfd_tresh = ref_vfd_buffer / ref_vfd_buff_size;
        ref_vfd_tresh -= adc_critical_conf.voltage_feedback_treshhold;
    }
}*/

IRAM_ATTR void read_voltage_feedback( volatile adc_readings &adc_data ){

    //if( ref_vfd_create ){ generate_ref_vfd( adc_data.voltage_channel_adc_recent ); }
...
void IRAM_ATTR G_EDM_SENSORS::set_vfd(){

    // pwm needs to be OFF
    // dpm needs to be ON
    // i2s needs to be running
    // i2s readout needs to be triggered
    // vfd needs to be collected x times 
    // maybe better integrate it into the pwm_controller..

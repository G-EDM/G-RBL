...
volatile int   operation_mode                   = 4;       // 4=wire, 1 = drill/sinker, 2 = reamer (just a constant up/down movement without progressing down)
...
volatile int   wire_spindle_speed               = 500;    // speed in step frequency (hz)
volatile int   wire_spindle_speed_probing       = 140;    // spindle speed for probing in step frequency (hz), probing just needs a very tiny motion to reduce errors due to dirt
...
axis_conf axis_configuration[MAX_N_AXIS] = { 
    ...
    { "A", DEFAULT_A_MAX_TRAVEL, 0.0, A_STEPS_PER_MM },
    { "B", 100.0, 0.0, 0 },
    { "C", 100.0, 0.0, 0 } 
  };
...
void GRBL_LIMITS::limits_go_home(uint8_t cycle_mask) {
    ...
    for (uint8_t idx = 0; idx < n_axis; idx++) {
        ...
        if (bit_istrue(cycle_mask, bit(idx))) {
            max_travel = MAX(max_travel, (HOMING_AXIS_SEARCH_SCALAR)*g_axis[idx]->max_travel_mm.get());
        }
    }
    bool     approach    = true;
    ...
    do {
        ...
        delay_ms( DEFAULT_HOMING_DEBOUNCE_DELAY );  // Delay to allow transient dynamics to dissipate.
        approach = !approach;
        pl_data->step_delay = approach ? process_speeds.HOMING_FEED : process_speeds.HOMING_SEEK;
        if (approach) {
            max_travel  = DEFAULT_HOMING_PULLOFF * HOMING_AXIS_LOCATE_SCALAR;
            homing_rate = process_speeds.HOMING_FEED;
        } else {
            max_travel  = DEFAULT_HOMING_PULLOFF;
    ...
    auto mask    = homing_dir_mask->get();
    auto pulloff = DEFAULT_HOMING_PULLOFF;
    for (uint8_t idx = 0; idx < n_axis; idx++) {

        auto steps = g_axis[idx]->steps_per_mm.get();
        if (cycle_mask & bit(idx)) {
            float travel = g_axis[idx]->max_travel_mm.get();

            float mpos   = g_axis[idx]->home_position.get();
...
float GRBL_LIMITS::limitsMaxPosition(uint8_t axis) {
    ...
    return bitnum_istrue(homing_dir_mask->get(), axis) ? mpos + g_axis[axis]->max_travel_mm.get() : mpos;
}

float GRBL_LIMITS::limitsMinPosition(uint8_t axis) {
    float mpos = g_axis[axis]->home_position.get();
    //float mpos = axis_settings[axis]->home_mpos->get();
    return bitnum_istrue(homing_dir_mask->get(), axis) ? mpos : mpos - g_axis[axis]->max_travel_mm.get();
}
...
bool __attribute__((weak)) GRBL_LIMITS::limitsCheckTravel(float* target) {
    ...
    for (idx = 0; idx < n_axis; idx++) {
        ...
        if ((target[idx] < limitsMinPosition(idx) || target[idx] > limitsMaxPosition(idx)) && g_axis[idx]->max_travel_mm.get() > 0) {
            return true;
        }
    }
    return false;

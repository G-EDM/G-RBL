...
Error autohome_at_center( const char *value ){
    ...
    for( int i = 0; i < N_AXIS; ++i ){
        mm_pos   = float( g_axis[i]->max_travel_mm.get() / 2 ) * -1; // 240/2*-1=-120
        step_pos = round( mm_pos * g_axis[i]->steps_per_mm.get() ); // -120*67=
        //step_pos          = round( pos * axis_settings[ i ]->steps_per_mm->get() );
        sys_position[i]   = step_pos;
        sys_axis_homed[i] = true;
    }
...
void make_grbl_commands() {
    ...
    new GrblCommand("HCT", "Home/O", autohome_at_center, anyState);
